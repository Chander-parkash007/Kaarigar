...
const CATEGORY_COLORS = [
  'from-blue-500 to-blue-600', 'from-yellow-400 to-orange-500', 'from-amber-600 to-yellow-700',
  'from-green-500 to-emerald-600', 'from-cyan-500 to-teal-600', 'from-sky-400 to-blue-500',
  'from-pink-500 to-rose-600', 'from-purple-500 to-violet-600', 'from-slate-500 to-gray-600',
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-10">
          <span className="inline-block bg-[#FF6B00]/10 text-[#FF6B00] text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wider mb-3">
            {tr('cat_services_badge')}
          </span>
          <h2 className="text-3xl font-bold text-[#1B3A6B]">{tr('cat_title')}</h2>
          <p className="text-gray-500 mt-2">{tr('cat_subtitle')}</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {CATEGORIES.map((cat, i) => (
            <button
              ...
              aria-label={lang === 'ur' ? cat.urdu : cat.label}
            >
              ...
              <div className="p-5 flex flex-col items-center text-center gap-2">
                ...
                <div>
                  <p className="font-semibold text-[#1B3A6B] text-sm leading-tight">
                    {lang === 'ur' ? cat.urdu : cat.label}
                  </p>
                  {lang === 'en' && <p className="text-gray-400 text-xs mt-0.5" dir="rtl" lang="ur">{cat.urdu}</p>}
                </div>
