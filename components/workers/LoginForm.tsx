...
export function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      else { setError(data.error || tr('login_invalid')) }
    } catch { setError(tr('login_network')) }
    finally { setLoading(false) }
  }

  return (
    <>
      <h1 className="text-2xl font-bold text-[#1B3A6B] mb-1">{tr('login_title')}</h1>
      <p className="text-gray-500 text-sm mb-6">{tr('login_subtitle')}</p>
