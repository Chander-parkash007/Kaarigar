...
export function ReviewForm({ workerId, workerName }: ReviewFormProps) {
  ...
  // Always fetch fresh — no cache — so verification status is up to date
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErrors({ form: tr('network_error') })
    } finally {
      setLoading(false)
    }
  }
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 text-center">
          ...
          <p className="text-blue-600 text-sm mb-4">
            {lang === 'ur'
              ? 'جعلی جائزوں کو روکنے کے لیے صرف تصدیق شدہ گاہک ہی جائزہ لکھ سکتے ہیں'
              : 'Only verified customers can write reviews — this prevents fake reviews'}
          </p>
        ...
        <div className="bg-yellow-50 border border-yellow-300 rounded-xl p-5 text-center">
          ...
          <p className="text-yellow-700 text-sm mb-4">
            {lang === 'ur'
              ...
              : 'Verify your email to write reviews. Takes only 2 minutes!'}
          </p>
          <div className="flex flex-col sm:flex-row gap-2 justify-center">
            ...
            <button onClick={fetchCustomer}
              className="text-sm text-gray-500 hover:text-gray-700 px-4 py-2 border border-gray-300 rounded-xl">
              {lang === 'ur' ? '🔄 تازہ کریں' : '🔄 Already verified? Refresh'}
            </button>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        {errors.form && <p className="text-red-600 text-sm bg-red-50 p-2 rounded-lg">{errors.form}</p>}
